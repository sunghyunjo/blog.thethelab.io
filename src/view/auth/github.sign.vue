  ...
  mounted() {
    setTimeout(async () => {
      ...
      if (_.isNil(user) || _.isEmpty(user)) {
        this.emo = '( - . - )+';
        this.msg = '로그인 되어 있지 않아요, 로그인 상태를 확인해주세요';
        return;
      }
      const url = `https://github.com/login/oauth/authorize?client_id=${githubConfig.githubClient}&redirect_uri=${githubConfig.callbackURI}&state=${githubConfig.state}`;
